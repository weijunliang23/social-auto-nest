<template>
  <div class="material-management page-view">
    ...
    >
      <div v-loading="previewLoading" class="preview-container" v-if="currentMaterial">
        <div v-if="isVideoFile(currentMaterial.filename)" class="video-preview">
          <video v-if="previewUrl" controls style="max-width: 100%; max-height: 60vh;">
            <source :src="previewUrl" :type="getVideoMimeType(currentMaterial.filename)">
            您的浏览器不支持视频播放
          </video>
          <el-empty v-else-if="!previewLoading" description="视频加载失败" />
        </div>
        <div v-else-if="isImageFile(currentMaterial.filename)" class="image-preview">
          <img
            v-if="previewUrl"
            :src="previewUrl"
            :alt="currentMaterial.filename"
        ...
        <div v-else class="file-info">
          ...
          <el-button type="primary" :loading="downloadLoading" @click="downloadFile(currentMaterial)">下载文件</el-button>
        </div>
      </div>
    </el-dialog>
  </div>
...
const handlePreview = async (material) => {
  ...
  if (!isVideoFile(material.filename) && !isImageFile(material.filename)) {
    return
  }

  previewLoading.value = true
  try {
    const blob = await materialApi.fetchMaterialBlob(material.file_path)
...
const downloadFile = async (material) => {
  ...
  try {
    const blob = await materialApi.fetchMaterialBlob(material.file_path, { download: true })
