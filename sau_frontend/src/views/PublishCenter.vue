    ...
    <div class="publish-content">
      <div class="tab-content-wrapper">
        ...
        >
          ...
          <!-- 发布类型：视频 / 图文 -->
          <div class="publish-kind-section">
            <h3>发布类型</h3>
            <el-radio-group v-model="tab.publishKind" @change="() => onPublishKindChange(tab)">
              <el-radio-button label="video">视频</el-radio-button>
              <el-radio-button label="note">图文</el-radio-button>
            </el-radio-group>
          </div>
          ...
          <el-dialog
            ...
            :title="currentUploadTab?.publishKind === 'note' ? '本地上传（图片）' : '本地上传（视频）'"
            width="600px"
            class="local-upload-dialog"
          >
            <el-upload
              :key="currentUploadTab?.publishKind || 'video'"
              class="video-upload"
              drag
              :auto-upload="true"
              ...
              multiple
              :accept="currentUploadTab?.publishKind === 'note' ? 'image/*' : 'video/*'"
              :headers="authHeaders"
            >
              <el-icon class="el-icon--upload"><Upload /></el-icon>
              <div class="el-upload__text">
                <template v-if="currentUploadTab?.publishKind === 'note'">
              ...
              <template #tip>
                <div class="el-upload__tip">
                  <template v-if="currentUploadTab?.publishKind === 'note'">
                    支持常见图片格式，可多选；抖音图文最多 35 张且不支持 GIF
                  </template>
                  <template v-else>
          ...
          <!-- 原创声明（仅视频） -->
          <div v-if="tab.publishKind === 'video'" class="original-section">
            <el-checkbox
          ...
          <!-- 草稿选项 (仅在视频号可见) -->
          <div v-if="tab.publishKind === 'video' && tab.selectedPlatform === 2" class="draft-section">
            <el-checkbox
              v-model="tab.isDraft"
          ...
          <!-- 标签 (仅在抖音视频可见) -->
          <div v-if="tab.publishKind === 'video' && tab.selectedPlatform === 3" class="product-section">
            <h3>商品链接</h3>
            <el-input
          ...
          <!-- 图文正文 -->
          <div v-if="tab.publishKind === 'note'" class="note-section">
            <h3>正文</h3>
            <el-input
              v-model="tab.noteBody"
              type="textarea"
              :rows="6"
              placeholder="图文正文（可选，按各平台规则填写）"
              maxlength="2000"
              show-word-limit
              class="note-input"
            />
            ...
            <div class="schedule-controls">
              ...
              <div v-if="tab.scheduleEnabled" class="schedule-settings">
                <div class="schedule-item">
                  <span class="label">{{ tab.publishKind === 'note' ? '每天发布条数：' : '每天发布视频数：' }}</span>
          ...
          <div class="action-buttons">
            ...
            <el-button
              size="small"
              type="success"
              @click="confirmPublish(tab, true)"
              :loading="tab.publishing || false"
              :disabled="tab.publishing || false"
            >
              {{ tab.publishing ? '发布中...' : '可视化发布' }}
            </el-button>
          </div>
        </div>
      </div>
...
// 平台列表 - 对应后端 type；图文不支持视频号 (2)
const PLATFORMS_VIDEO = [
  { key: 3, name: '抖音' },
  { key: 4, name: '快手' },
  { key: 2, name: '视频号' },
  { key: 1, name: '小红书' }
...
const platformsForKind = (tab) => (tab.publishKind === 'note' ? PLATFORMS_NOTE : PLATFORMS_VIDEO)

const defaultTabInit = {
  name: 'tab1',
  label: '发布1',
  publishKind: 'video', // video | note
  fileList: [], // 后端返回的文件名列表
...
const onPublishKindChange = (tab) => {
  tab.fileList = []
  tab.displayFileList = []
  if (tab.publishKind === 'note' && tab.selectedPlatform === 2) {
    tab.selectedPlatform = 1
    ElMessage.info('图文不支持视频号，已切换为小红书')
  }
}
...
const confirmPublish = async (tab, browserPublish = false, { redirectOnSuccess = true } = {}) => {
  ...
  if (tab.fileList.length === 0) {
    ElMessage.error(tab.publishKind === 'note' ? '请先上传图片' : '请先上传视频文件')
    tab.publishing = false
    throw new Error(tab.publishKind === 'note' ? '请先上传图片' : '请先上传视频文件')
  }
  if (!tab.title.trim()) {
  ...
  if (tab.publishKind === 'note' && tab.selectedPlatform === 2) {
    ElMessage.error('图文不支持视频号')
    tab.publishing = false
    throw new Error('图文不支持视频号')
  }
  ...
  try {
    ...
    const { ok, record } = await waitAndHandlePublishJob(recordId, {
      accountStore,
      router,
      fallbackAccountIds: tab.selectedAccounts,
      successLabel
    })
...
const confirmMaterialSelection = () => {
  ...
  if (currentUploadTab.value) {
    const kind = currentUploadTab.value.publishKind
    // 将选中的素材添加到当前tab的文件列表
    selectedMaterials.value.forEach(materialId => {
      const material = materials.value.find(m => m.id === materialId)
      if (material) {
        const baseName = material.filename || material.file_path || ''
        if (kind === 'note' && !IMAGE_EXT.test(baseName)) {
          return
        }
        if (kind === 'video' && IMAGE_EXT.test(baseName)) {
          return
        }
        const fileInfo = {
          ...
          type: kind === 'note' ? 'image/*' : 'video/mp4'
        }
...
const batchPublish = async () => {
  ...
  try {
    for (let i = 0; i < tabs.length; i++) {
      ...
      try {
        await confirmPublish(tab, false, { redirectOnSuccess: false })
        publishResults.value.push({
          label: tab.label,
          status: 'success',
          message: '发布成功'
  ...
  .publish-center {
    ...
    .publish-content {
      ...
      .tab-content-wrapper .tab-content {
        .schedule-controls .schedule-settings .schedule-item {
          flex-direction: column;
          align-items: flex-start;
          gap: 8px;
    ...
    .uploaded-files .file-list .file-item {
      flex-wrap: wrap;
      gap: 8px;
    ...
    .topic-dialog .topic-dialog-content .recommended-topics .topic-grid {
